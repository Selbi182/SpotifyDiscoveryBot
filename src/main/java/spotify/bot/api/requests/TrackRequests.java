...
public class TrackRequests {
	 ...
	 * @throws Exception 
	 */
	public static List<List<TrackSimplified>> getSongIdsByAlbums(SpotifyApiSessionManager api, List<Album> albums) throws Exception {
		...
		for (Album a : albums) {
			List<TrackSimplified> currentList = SpotifyApiRequest.execute(new Callable<List<TrackSimplified>>() {
				...
				public List<TrackSimplified> call() throws Exception {
					...
					do {
						GetAlbumsTracksRequest.Builder request = api.api().getAlbumsTracks(a.getId()).limit(Constants.DEFAULT_LIMIT);
	 ...
	 * @throws Exception 
	 */
	public static List<List<TrackSimplified>> findFollowedArtistsSongsOnAlbums(SpotifyApiSessionManager api, List<Album> newAlbums, List<Artist> followedArtists) throws Exception {
		...
		List<List<TrackSimplified>> newSongs = getSongIdsByAlbums(api, newAlbumsWithoutCompilations);
