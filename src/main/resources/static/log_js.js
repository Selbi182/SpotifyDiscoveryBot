(function () {
  ...
  let clearNotifications = document.getElementById("clear-notifications");
  clearNotifications.onclick = () => {
    clearNotifications.classList.add("no-more");
    fetch(`/clear`)
      .then(response => {
        if (response.status >= 200 && response.status < 300) {
          clearNotifications.innerHTML = "Notification markers cleared!";
        }
      })
      .catch(ex => console.error("Error while clearing notification markers", ex));
  }
